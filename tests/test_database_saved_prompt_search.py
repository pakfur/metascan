...
def db():
    with tempfile.TemporaryDirectory() as d:
        ...
        manager.create_folder(
            "fld_a", kind="manual", name="A",
            items=["/data/a/img1.png", "/data/a/img2.png"],
        )
        manager.create_folder("fld_b", kind="manual", name="B", items=["/data/b/img3.png"])
        manager.create_folder("fld_smart", kind="smart", name="Smart", rules={"any": []})

        # Saved prompts: 2 against fld_a/img1 (one qwen, one sd), 1 against fld_a/img2 (qwen),
        # 1 against fld_b/img3 (qwen).
        manager.save_prompt(file_path="/data/a/img1.png", name="hero",
                            prompt="p1", target_model="qwen", architecture="qwen",
                            styles=[], mode="generate",
                            temperature=None, max_tokens=None, source_prompt=None,
                            negative=None, vlm_model_id=None)
        manager.save_prompt(file_path="/data/a/img1.png", name="alt",
                            prompt="p2", target_model="sd", architecture="sd",
                            styles=[], mode="generate", negative="n2",
                            temperature=None, max_tokens=None, source_prompt=None,
                            vlm_model_id=None)
        manager.save_prompt(file_path="/data/a/img2.png", name="cinematic",
                            prompt="p3", target_model="qwen", architecture="qwen",
                            styles=[], mode="generate",
                            temperature=None, max_tokens=None, source_prompt=None,
                            negative=None, vlm_model_id=None)
        manager.save_prompt(file_path="/data/b/img3.png", name="landscape",
                            prompt="p4", target_model="qwen", architecture="qwen",
                            styles=[], mode="generate",
                            temperature=None, max_tokens=None, source_prompt=None,
                            negative=None, vlm_model_id=None)
        yield manager
...
def test_returned_rows_have_styles_deserialized_to_list(db):
    ...
    rows = db.search_saved_prompts(folder_id=None, target_model="qwen", name="hero", limit=10)
    assert len(rows) == 1

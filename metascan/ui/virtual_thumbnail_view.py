...
from PyQt6.QtCore import (
    ...
    QEasingCurve, QParallelAnimationGroup, QObject
)
from PyQt6.QtGui import (
    QPixmap, QFont, QPainter, QPen, QColor, QBrush, QPolygon, QPolygonF, 
    QDesktopServices, QKeySequence, QShortcut, QPaintEvent, QResizeEvent,
...
class LayoutMetrics:
    ...
    margins: Tuple[int, int, int, int] = field(default_factory=lambda: (10, 10, 10, 10))  # left, top, right, bottom
...
class WidgetPool:
    ...
    def _configure_widget(self, widget: ThumbnailWidget, media: Media) -> None:
        ...
        widget.setToolTip(f"Click to select • Double-click to open {media_type}\n{media.file_name}")
...
class VirtualScrollArea(QScrollArea):
    ...
    def apply_filters(self, filtered_paths: Set[str]) -> None:
        ...
        if not filtered_paths:
            ...
            # Filter media list
            self.filtered_media = [
                media for media in self.media_list 
                if str(media.file_path) in filtered_paths
            ]
        
        # Clear selection if the selected item is no longer visible
        if (self.selected_media and 
            str(self.selected_media.file_path) not in {str(m.file_path) for m in self.filtered_media}):
            self.selected_media = None
            self.selected_index = -1
    ...
    def _calculate_layout(self) -> None:
        ...
        available_width = viewport_width - self.layout_metrics.margins[0] - self.layout_metrics.margins[2]
    ...
    def _update_scroll_range(self) -> None:
        ...
        if self.layout_metrics.rows == 0:
            ...
            total_height = (
                self.layout_metrics.margins[1] +  # top margin
    ...
    def _update_viewport(self) -> None:
        ...
        first_visible_row = max(0, (scroll_y - self.layout_metrics.margins[1]) // cell_height - buffer_rows)
        last_visible_row = min(
            self.layout_metrics.rows - 1,
            ((scroll_y + visible_height - self.layout_metrics.margins[1]) // cell_height) + buffer_rows
        )
        ...
        last_item = min(len(self.filtered_media) - 1, (last_visible_row + 1) * columns - 1)
        ...
        logger.debug(f"Viewport updated: rows {first_visible_row}-{last_visible_row}, "
    ...
    def _hide_widget_at_index(self, index: int) -> None:
        ...
        if index in self.visible_widgets:
            ...
            media = self.filtered_media[index] if index < len(self.filtered_media) else None
    ...
    def _position_widget(self, widget: ThumbnailWidget, index: int) -> None:
        ...
        x = (
            ...
            col * self.layout_metrics.cell_width
        )
        y = (
            self.layout_metrics.margins[1] +
            row * self.layout_metrics.cell_height
        )
        
        widget.setGeometry(
            x, y,
            self.layout_metrics.item_width,
            self.layout_metrics.item_height
        )
    ...
    def _load_thumbnail_for_widget(self, widget: ThumbnailWidget) -> None:
        ...
        if not widget.pixmap() or widget.pixmap().isNull():
            ...
            if self.thumbnail_cache:
                thumbnail_path = self.thumbnail_cache.get_thumbnail_path(widget.media.file_path)
    ...
    def _on_thumbnail_ready(self, media: Media, thumbnail_path: Optional[Path]) -> None:
        ...
        for index, widget in self.visible_widgets.items():
            if (index < len(self.filtered_media) and 
                self.filtered_media[index].file_path == media.file_path):
...
class VirtualThumbnailView(QWidget):
    ...
    def set_media_list(self, media_list: List[Media]) -> None:
        ...
        Args:
            media_list: List of Media objects to display
        """
        logger.info(f"VirtualThumbnailView: Setting media list with {len(media_list)} items")
    ...
    def apply_filters(self, filtered_paths: Set[str]) -> None:
        ...
        Args:
            filtered_paths: Set of file paths to show (empty set shows all)
        """
        logger.debug(f"VirtualThumbnailView: Applying filters to {len(filtered_paths)} paths")
    ...
    def keyPressEvent(self, event) -> None:
        ...
        if key == Qt.Key.Key_Left:
            ...
            visible_rows = self.scroll_area.viewport_info.visible_height // self.scroll_area.layout_metrics.cell_height

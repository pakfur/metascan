...
from PyQt6.QtWidgets import (
    ...
    QSlider, QStackedWidget, QSizePolicy
)
from PyQt6.QtCore import (
    Qt, QTimer, pyqtSignal, QUrl, QSize, QPoint
)
from PyQt6.QtGui import (
    QPixmap, QFont, QKeyEvent, QPainter, QImage, QAction, QShortcut, QKeySequence
)
...
class MediaViewer(QWidget):
    ...
    def _display_current_media(self):
        ...
        if self.current_media.is_video:
            ...
            if not success:
                self.info_label.setText(f"Failed to load video: {self.current_media.file_name}")
        else:
            # Show image viewer
            self.stacked_widget.setCurrentWidget(self.image_viewer)
            success = self.image_viewer.load_image(self.current_media.file_path)
            if not success:
                self.info_label.setText(f"Failed to load image: {self.current_media.file_name}")

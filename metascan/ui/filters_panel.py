from PyQt6.QtWidgets import (
    ...
    QScrollArea, QCheckBox, QFrame, QSizePolicy, QLineEdit
)
...
class FilterSection(QFrame):
    ...
    def toggle_section(self):
        ...
        self.header_button.setText(f"{icon} {self.section_name.title()} ({len(self.items_list)})")
    ...
    def filter_items(self, filter_text: str):
        ...
        # Update header count
        icon = "▼" if self.is_expanded else "▶"
        self.header_button.setText(f"{icon} {self.section_name.title()} ({len(filtered_items)})")
...
class FiltersPanel(QWidget):
    ...
    def update_filters(self, filter_data: Dict[str, List[Dict[str, Any]]]):
        ...
        current_prompt_filter = self.prompt_filter_edit.text() if self.prompt_filter_edit else ""

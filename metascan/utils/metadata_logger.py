...
class MetadataParsingLogger:
    ...
    def log_extraction_attempt(
        ...
        if not success and error:
            self._log_to_csv(
                timestamp, file_id, file_path, extractor_name, error, raw_data
            )
...
class MetadataLogAnalyzer:
    ...
    def analyze_all(self) -> Dict[str, Any]:
        ...
        if not errors:
            return {"total_errors": 0, "message": "No errors found in logs"}

        # Aggregate analysis
        files_with_errors = set()
        error_types: Dict[str, int] = {}
        extractor_failures: Dict[str, int] = {}
    ...
    def retest_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        original_logger = manager.parsing_logger
        manager.parsing_logger = self.logger

...
def clear_logs(args):
    ...
    if not args.force:
        response = input(
            "Are you sure you want to clear all metadata parsing logs? (y/N): "
        )
        if response.lower() != "y":
            print("Cancelled")
            return
...
def show_stats(args):
    ...
    if result.get("error_types"):
        ...
        sorted_errors = sorted(
            result["error_types"].items(), key=lambda x: x[1], reverse=True
        )
...
Examples:
  ...
  %(prog)s clear-logs --force
    ...
    analyze_all_parser = subparsers.add_parser(
        "analyze-all", help="Analyze all errors in the log"
    )
    ...
    analyze_file_parser = subparsers.add_parser(
        "analyze-file", help="Analyze errors for a specific file"
    )
    analyze_file_parser.add_argument("file", help="Path to the media file")
    ...
    retest_parser = subparsers.add_parser(
        "retest", help="Re-run metadata extraction on a file"
    )
    retest_parser.add_argument("file", help="Path to the media file")
    ...
    # Show CSV
    csv_parser = subparsers.add_parser("csv", help="Export errors in CSV format")
    csv_parser.add_argument("--file-id", help="Filter by file ID")

    # Clear logs
    clear_parser = subparsers.add_parser("clear-logs", help="Clear all log files")

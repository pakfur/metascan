...
class EmbeddingQueue(QObject):
    ...
    def start_indexing(
        ...
        with open(temp_file, "w") as f:
            json.dump(task, f)
        os.replace(str(temp_file), str(task_file))
    ...
    def poll_updates(self) -> None:
        ...
        if status == "loading_model":
            ...
            self.progress_updated.emit(current, total, "Indexing cancelled")
            self._cleanup()
        elif status == "error":
        ...
        if self._process is not None and self._process.poll() is not None:
            if status not in ("complete", "cancelled", "error"):
                stderr = ""
                try:
                    stderr = self._process.stderr.read() if self._process.stderr else ""
                except Exception:
                    pass
                self.indexing_error.emit(f"Worker exited unexpectedly: {stderr}")
                self._cleanup()

    def _cleanup(self) -> None:
        """Clean up after the worker finishes."""
        self._process = None
        # Clean up task file but leave progress for final read
        task_file = self._queue_dir / "embedding_task.json"
        if task_file.exists():
            try:
                task_file.unlink()

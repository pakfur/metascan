...
class _Node:
    ...
    raw: Dict[str, Any] = None  # original node JSON (optional for advanced heuristics)
...
def _node_widgets(n: Dict[str, Any], widget_idx_map: Optional[Dict[str, Dict[str, int]]] = None) -> Dict[str, Any]:
    ...
    for k in ("seed", "steps", "cfg", "sampler_name", "scheduler", "denoise", "width", "height", "text", "filename", "fps", "frame_rate"):
        if k in n:
...
def _build_graph(workflow: Dict[str, Any]) -> _Graph:
    ...
    widget_idx_map = workflow.get("widget_idx_map") if isinstance(workflow, dict) else None
    ...
    if isinstance(workflow.get("nodes"), list):
        ...
        raw_nodes = list(workflow.values()) if isinstance(workflow, dict) and all(isinstance(v, dict) for v in workflow.values()) else []
        style = "prompt"
    ...
    for n in raw_nodes:
        ...
        nodes[nid] = _Node(id=nid, type=ntype, inputs=inputs_map, widgets=widgets, title=n.get("title"), raw=n)
    ...
    if style == "workflow":
        ...
        if isinstance(links, list):
            for l in links:
                ...
                src_id = str(src_id); dst_id = str(dst_id)
                src_slot_name = None
                dst_slot_name = None
                # Try to resolve names from node port arrays
                try:
                    if src_id in nodes:
                        outs = nodes[src_id].raw.get("outputs") or []
                        if isinstance(outs, list) and isinstance(src_slot_idx, int) and 0 <= src_slot_idx < len(outs):
                            src_slot_name = _as_str_name(outs[src_slot_idx], str(src_slot_idx))
                except Exception:
                    pass
                try:
                    if dst_id in nodes:
                        ins = nodes[dst_id].raw.get("inputs") or []
                        if isinstance(ins, list) and isinstance(dst_slot_idx, int) and 0 <= dst_slot_idx < len(ins):
                            dst_slot_name = _as_str_name(ins[dst_slot_idx], str(dst_slot_idx))
                except Exception:
                    pass
                ...
                edges.append(_Edge(src=_Pin(src_id, src_slot_name), dst=_Pin(dst_id, dst_slot_name), etype=_infer_edge_type(dst_slot_name)))
        else:
            # Fallback: synthesize edges via per-node 'inputs' lists if they embed link ids
            for n in raw_nodes:
                dst_id = str(n.get("id") or n.get("_id") or n.get("uid") or n.get("name"))
                ins = n.get("inputs") or []
                ...
                for i, inp in enumerate(ins):
                    ...
                    edges.append(_Edge(src=_Pin("?", "?"), dst=_Pin(dst_id, dst_slot_name), etype=_infer_edge_type(dst_slot_name)))
    else:
        ...
        for n in raw_nodes:
            ...
            if isinstance(inputs, dict):
                for slot, conn in inputs.items():
                    ...
                    conns = conn if (isinstance(conn, list) and conn and isinstance(conn[0], list)) else [conn]
                    for c in conns:
                        if isinstance(c, list) and len(c) >= 2:
                            src_id = str(c[0]); src_slot = _coerce_str(c[1])
                            edges.append(_Edge(src=_Pin(src_id, src_slot), dst=_Pin(nid, str(slot)), etype=_infer_edge_type(str(slot))))
...
def _find_terminals(g: _Graph) -> List[str]:
    ...
    for nid, node in g.nodes.items():
        nt = node.type.lower()
        if any(t in nt for t in ["saveimage", "imagesave", "previewimage", "savevideo", "videosave", "videocombine"]):
            out.append(nid)
    return out
...
class ComfyUIMetadataExtractor(MetadataExtractor):
    ...
    def can_extract(self, media_path: Path) -> bool:
        ...
        try:
            ...
            if suffix in {".png", ".webp"}:
                ...
                for k, v in md.items():
                    lk = str(k).lower(); sv = v.decode("utf-8", "ignore") if isinstance(v, (bytes, bytearray)) else str(v)
                    if any(sig in lk for sig in ("prompt", "workflow")) and ("{" in sv and "nodes" in sv or '"class_type"' in sv):
                        return True
    ...
    # ------ Internal helpers ------
    def _load_comfy_payload(self, media_path: Path, parsing_errors: List[Dict[str, Any]] ) -> Tuple[Optional[Dict[str, Any]], Dict[str, Any]]:
        ...
        try:
            if media_path.suffix.lower() in {".png", ".webp"}:
                ...
                if png_md:
                    raw_meta["png_text"] = {k: (v.decode("utf-8", "ignore") if isinstance(v, (bytes, bytearray)) else v) for k, v in png_md.items()}
                    ...
                    for key in ordered_keys:
                        ...
                        try:
                            ...
                            try:
                                ...
                                parsing_errors.append({"error_type": type(e).__name__, "error_message": str(e), "raw_data": str(s)[:2000]})
        except Exception as e:
            parsing_errors.append({"error_type": type(e).__name__, "error_message": f"PNG read failed: {e}", "raw_data": "<binary>"})

        if payload is None and media_path.suffix.lower() in {".jpg", ".jpeg"}:
            try:
                exif_md = self._get_exif_metadata(media_path)
                raw_meta["exif"] = {k: (v if isinstance(v, (str, int, float)) else str(v)) for k, v in exif_md.items()}
                uc = exif_md.get("UserComment")
                if isinstance(uc, str) and ("{" in uc and "nodes" in uc):
                    try:
                        payload = json.loads(uc)
                    except Exception as e:
                        parsing_errors.append({"error_type": type(e).__name__, "error_message": str(e), "raw_data": uc[:2000]})
            except Exception as e:
                parsing_errors.append({"error_type": type(e).__name__, "error_message": f"EXIF read failed: {e}", "raw_data": "<binary>"})

        if payload is None and media_path.suffix.lower() in self.VIDEO_EXTS:
            try:
                payload, tags_meta = self._probe_video_for_comfy_payload(media_path, return_tags=True)
                if tags_meta:
                    raw_meta["video_tags"] = tags_meta
            except Exception as e:
                parsing_errors.append({"error_type": type(e).__name__, "error_message": f"Video tag probe failed: {e}", "raw_data": str(media_path)})

        return payload, raw_meta

    def _select_terminal(self, terminals: List[str], graph: _Graph, media_path: Path) -> Optional[str]:
        if not terminals:
            return None
        if len(terminals) == 1:
            return terminals[0]
        ...
        for tid in terminals:
            ...
            for key in ("filename", "file", "name", "pattern", "w0"):
                if key in w and isinstance(w[key], str) and base in w[key].lower():
                    return tid
        for tid in terminals:
            if any(x in graph.nodes[tid].type.lower() for x in ("saveimage", "savevideo", "videocombine")):
                return tid
        return terminals[0]

    def _apply_known_adapters(self, node: _Node, sub: _Graph, state: Dict[str, Any]) -> None:
        ntype = node.type.lower()
        # Checkpoint / Model loader
        if "checkpoint" in ntype or ("model" in node.widgets and any(k in node.widgets for k in ("ckpt_name", "model", "model_name", "file", "w0"))):
            model = self._first_string(node.widgets, ["ckpt_name", "model", "model_name", "file", "w0"]) or self._first_path_like(node.widgets)
            if model:
                state["model"] = Path(str(model)).name
            return
        # CLIP text encode
        if "cliptextencode" in ntype or ("text" in node.widgets and "clip" in node.inputs):
            txt = self._first_string(node.widgets, ["text"]) or self._first_text_like(node.widgets)
            if txt:
        ...
        if "sampler" in ntype or "ksampler" in ntype:
            ...
            state["seed"] = self._prefer(state.get("seed"), self._to_int(w.get("seed") or w.get("w0")))
            state["steps"] = self._prefer(state.get("steps"), self._to_int(w.get("steps") or w.get("w2")))
            state["cfg_scale"] = self._prefer(state.get("cfg_scale"), self._to_float(w.get("cfg") or w.get("w3")))
            state["sampler"] = self._prefer(state.get("sampler"), self._to_str(w.get("sampler_name") or w.get("w4")))
            state["scheduler"] = self._prefer(state.get("scheduler"), self._to_str(w.get("scheduler") or w.get("w5")))
            return
        # Latent init / base size
        if "emptylatentimage" in ntype or "latents" in ntype or ("width" in node.widgets and "height" in node.widgets):
            w = node.widgets
            state["width"] = self._prefer(state.get("width"), self._to_int(w.get("width") or w.get("w0")))
            state["height"] = self._prefer(state.get("height"), self._to_int(w.get("height") or w.get("w1")))
            if "batch" in w or "batch_size" in w or "w2" in w:
                state["batch_size"] = self._prefer(state.get("batch_size"), self._to_int(w.get("batch") or w.get("batch_size") or w.get("w2")))
            return
        # VAE clip_skip sometimes exposed as widget
        if "vae" in ntype and "loader" in ntype:
            if "clip_skip" in node.widgets:
                state["clip_skip"] = self._prefer(state.get("clip_skip"), self._to_int(node.widgets.get("clip_skip")))
            return
                ...
                if isinstance(v, list):
                    for item in v:
                        if isinstance(item, dict) and item.get("on") and item.get("lora"):
                            ...
                            state.setdefault("loras", []).append({"lora_name": name, "lora_weight": weight})
                            added = True
            if not added:
                name = self._first_path_like(node.widgets) or self._first_string(node.widgets, ["lora", "file", "w0"]) or "lora"
                # Prefer UNet weight keys, then text weight
                unet_w = self._to_float(self._first_number_like(node.widgets, ["strength_model", "unet_weight", "strength", "w1"]))
                text_w = self._to_float(self._first_number_like(node.widgets, ["strength_clip", "text_weight", "w2"]))
                weight = unet_w if unet_w is not None else (text_w if text_w is not None else 1.0)
                state.setdefault("loras", []).append({"lora_name": Path(str(name)).name, "lora_weight": float(weight) if weight is not None else 1.0})
            return
        ...
        if any(k in ntype for k in ("scale", "resize", "upscale")):
            if "width" in node.widgets and "height" in node.widgets:
                state["width"] = self._prefer(state.get("width"), self._to_int(node.widgets.get("width")))
                state["height"] = self._prefer(state.get("height"), self._to_int(node.widgets.get("height")))
            return
        # Video save / combine (fps/frame_rate)
        if any(k in ntype for k in ("savevideo", "videosave", "videocombine")):
            if "fps" in node.widgets:
                state["frame_rate"] = self._prefer(state.get("frame_rate"), self._to_float(node.widgets.get("fps")))
            if "frame_rate" in node.widgets:
                state["frame_rate"] = self._prefer(state.get("frame_rate"), self._to_float(node.widgets.get("frame_rate")))
            ...
            try:
                ...
                if fr is not None:
                    state["frame_rate"] = self._prefer(state.get("frame_rate"), self._to_float(fr))
            except Exception:
                pass
            return
    ...
    def _heuristic_fallback(self, node: _Node, sub: _Graph, state: Dict[str, Any]) -> None:
        w = node.widgets
        ...
        if txt and len(txt) < 8000:
            ...
            state[bucket].append(str(txt))
        # Sampler-ish values
        for k, target in [("seed", "seed"), ("steps", "steps"), ("cfg", "cfg_scale"), ("sampler", "sampler"), ("sampler_name", "sampler"), ("scheduler", "scheduler")]:
        ...
        for key, val in w.items():
            if isinstance(val, str) and ("lora" in val.lower()) and (val.endswith((".safetensors", ".pt"))):
                state.setdefault("loras", []).append({"lora_name": Path(val).name, "lora_weight": 1.0})
                break
    ...
    def _probe_video_for_comfy_payload(self, media_path: Path, return_tags: bool = False):
    ...
    def _json_from_tag_dict(self, tags: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for k, v in (tags or {}).items():
            ...
            lk = k.lower()
            if lk in ("prompt", "workflow", "description", "comment", "comfyui_prompt", "comfyui_workflow") or ("prompt" in lk or "workflow" in lk):
                candidates.append(s)
                ...
                for attempt in (s, self._try_json_string(s)):
                    try:
                        ...
                        if isinstance(j, dict):
                            if "nodes" in j or any(isinstance(val, dict) and "class_type" in val for val in j.values()):
                                if "nodes" in j:
                                    return j
                                for key in ("prompt", "workflow"):
                                    if isinstance(j.get(key), dict) and ("nodes" in j[key] or any(isinstance(val, dict) and "class_type" in val for val in j[key].values())):
                                        return j[key]
                    except Exception:
                        continue
        return None
    ...
    def _probe_video_metadata(self, media_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if streams:
                    s = streams[0]
                    fps = self._parse_rate(s.get("avg_frame_rate")) or self._parse_rate(s.get("r_frame_rate"))
                    if fps:
                        out["fps"] = fps
                    if s.get("nb_frames") is not None:
                        try:
                            out["frames"] = int(s["nb_frames"]) if str(s["nb_frames"]).isdigit() else None
                        except Exception:
                            pass
            ...
            try:
                ...
                if fps:
                    out["fps"] = float(fps)
                nframes = meta.get("nframes") or (meta.get("duration") and out.get("fps") and int(meta["duration"] * out["fps"])) or None
                if nframes:

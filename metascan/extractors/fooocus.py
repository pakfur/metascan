...
class FooocusExtractor(MetadataExtractor):
    ...
    def _is_parameter_line(self, line: str) -> bool:
        ...
        return bool(re.search(r'(Steps|Sampler|CFG scale|Seed|Size|Model|LoRAs?):', line))
    
    def _parse_parameter_line(self, line: str, extracted: Dict[str, Any], loras: list = None):
        ...
        parts = re.split(r',(?=\s*(?:Steps|Sampler|CFG scale|Seed|Size|Model|Width|Height|LoRAs?):)', line)
    ...
    def _extract_from_json(self, data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if isinstance(loras_data, list):
                for lora_item in loras_data:
                    if isinstance(lora_item, dict):
                        ...
                        lora_weight = lora_item.get("weight") or lora_item.get("strength") or lora_item.get("model_strength", 1.0)
        ...
        lora_keys = [k for k in data.keys() if k.startswith("lora") and not k.endswith(("_weight", "_strength"))]

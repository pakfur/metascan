...
class ThumbnailCache:
    ...
    def _create_thumbnail(
        self, media_path: Path, thumbnail_path: Path
    ) -> Optional[Path]:
    ...
    def _create_image_thumbnail(
        self, image_path: Path, thumbnail_path: Path
    ) -> Optional[Path]:
    ...
    def _create_video_thumbnail(
        self, video_path: Path, thumbnail_path: Path
    ) -> Optional[Path]:
        """Create a thumbnail for a video file"""
        try:
            # Try python-ffmpeg bindings first
            if HAS_FFMPEG_PYTHON and self._create_video_thumbnail_python(
    ...
    def _create_video_thumbnail_python(
        self, video_path: Path, thumbnail_path: Path
    ) -> bool:
        ...
        try:
            ...
            stream = ffmpeg.output(
                stream, str(thumbnail_path), vframes=1, format="image2"
            )
            ...
            if thumbnail_path.exists():
                logger.debug(
                    f"Created video thumbnail using python-ffmpeg for {video_path}"
                )
                return True
            ...
            logger.debug(
                f"python-ffmpeg failed for {video_path}: {e.stderr.decode() if e.stderr else str(e)}"
            )
            return False
        except Exception as e:
    ...
    def _create_video_thumbnail_ffmpeg(
        self, video_path: Path, thumbnail_path: Path
    ) -> bool:
    ...
    def _create_video_thumbnail_fallback(
        self, video_path: Path, thumbnail_path: Path
    ) -> Optional[Path]:
    ...
    def create_thumbnails_batch(
        self, media_paths: List[Path], callback=None
        ...
        for thumbnail in self.cache_dir.glob("*.jpg"):
            if thumbnail.name not in valid_thumbnails:
                try:
                    ...
                    logger.error(
                        f"Failed to remove orphaned thumbnail {thumbnail}: {e}"
                    )

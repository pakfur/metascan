...
| `qwen3vl-8b` | CUDA < 10 GB VRAM; CPU-only (needs GPU acceleration) | `cuda_workstation` below the 22 GB `qwen38-27b` floor |
| `qwen3vl-30b-a3b` (MoE) | CUDA < 24 GB VRAM; CPU-only | Never — available from 24 GB up, but `qwen38-27b` is preferred at that tier |
| `qwen38-27b` | CUDA < 22 GB VRAM; CPU-only | `cuda_workstation` ≥ 22 GB VRAM |
